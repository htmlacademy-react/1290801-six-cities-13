...
function MainPage({offers}: MainPageProps): JSX.Element {
	...
	const uniqueCities: string[] = Array.from(new Set(offers.map((offer) => offer.city.name))).map((city) => city.toLowerCase());
	const currentCity: string = urlCity || uniqueCities[0];
	const currentOffers = offers.filter((offer) => offer.city.name.toLowerCase() === currentCity);
	return (
		(!urlCity || uniqueCities.find((uniqCity) => uniqCity === urlCity)) ?
